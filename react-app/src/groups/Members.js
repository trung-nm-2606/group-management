...
const Members = () => {
  ...
  const [addingMemberError, setAddingMemberError] = useState(null);
    ...
    axios
      ...
      .then((res) => {
        ...
        if (status) {
          ...
          setAddingMemberError(error);
        }
        setAddingMember(false);
      })
    ;
  }, [newMemberEmail, setAddingMember, setMembers, setNewMemberEmail, setAddingMemberError]);

  return (
    <>
      {addingMemberError && (
        <Alert severity="error" sx={{ marginBottom: 1 }}>{addingMemberError}</Alert>
      )}
